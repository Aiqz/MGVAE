    ...
    def __init__(self, root: str, train: bool = True, transform: Optional[Callable] = None, target_transform: Optional[Callable] = None, download: bool = False,
        ...
        if train:
            ...
            if aug == 'smote':
                ...
                if exemplar_model == None:
                    raise NotImplementedError("No exemplar model!")
                aug_data, aug_label = cond_gan_aug(
                    self.data, self.targets, model=exemplar_model)
                self.data = torch.cat(
                    (self.data, aug_data.detach().cpu()), dim=0)
                self.targets = torch.cat(
...
class MNISTDataset(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "fit" or stage is None:
            ...
            elif self.aug_way == 'cgan':
                self.mnist_train = MyMNIST(self.data_dir, train=True, transform=transform,
                                           my_target=self.target_label, downsample_size=self.downsample_size,
                                           aug='cgan', exemplar_model=self.exemplar_model)
        ...
        if stage == "validate" or stage is None:
            ...
            self.mnist_val = MyMNIST(self.data_dir, train=False, transform=transform,
                                     my_target=self.target_label, downsample_size=self.downsample_size,
                                     test_downsample_size=0)

        if stage == "test" or stage is None:
            # self.mnist_test = MNIST(self.data_dir, train=False, transform=transform)
            self.mnist_test = MyMNIST(self.data_dir, train=False, transform=transform,
                                      my_target=self.target_label, downsample_size=self.downsample_size,
                                      test_downsample_size=0)
    ...
    def train_dataloader(self):
        if self.aug_way == 'rs':
            return DataLoader(self.mnist_train,
                              ...
                              sampler=WeightedRandomSampler(self.mnist_train.train_in_idx, len(self.mnist_train.train_in_idx)),
