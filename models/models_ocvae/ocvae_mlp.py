...
class Condition_VAE_MLP(pl.LightningModule):
    def __init__(self,
                 num_channels: int = 1,
                 img_size: int = 28,
                 latent_dim: int = 40,
                 ...
                 params:dict = None,
                 **kwargs) -> None:
        super(Condition_VAE_MLP, self).__init__()
        ...
        self.encoder = nn.Sequential(
                nn.Linear(num_channels * img_size * img_size + num_classes, self.hidden_size),
                nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(self.hidden_size, self.hidden_size),
            nn.ReLU(),
            nn.Linear(self.hidden_size, num_channels * img_size * img_size),
            nn.Sigmoid(),
            )
    ...
    def decode(self, z):
        ...
        result = result.view(-1, self.num_channels, self.img_size, self.img_size)
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        ...
        x = torch.cat([input.view(-1, 784), y], dim = 1)
