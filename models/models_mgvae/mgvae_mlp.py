...
class MGVAE_MLP(nn.Module):
    ...
    def encode(self, input: Tensor) -> List[Tensor]:
        ...
        """
        result = self.encoder(input.view(-1, 784))
    ...
    def loss_function(self,
        ...
        RE = -F.mse_loss(input.view(-1, 784), recons.view(-1, 784), reduction='none')
        RE = torch.sum(RE, dim=1)
    ...
    def get_exemplar_set(self):
        exemplar_data = self.dataset_loader.dataset.data / 255.0
